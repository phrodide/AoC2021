...
namespace AoC2021.Days
{
    ...
    internal class Day5
    {
        ...
        public string OriginalSolvePart1()
        {
            ...
            foreach (var line in lines)
            {
                int[] parts = line.Replace(" -> ",",").Split(',').Select(int.Parse).ToArray();
                //parts0 and 1 are start, 2 and 3 are end.
                if (parts[0]==parts[2])
                {
                    //this is a vertical pull
                    for (int i = Math.Min(parts[1],parts[3]); i <= Math.Max(parts[1],parts[3]); i++)
                    {
                        grid[(parts[0], i)] += 1;
                    }
                }
                ...
                {
                    //this is a horizontal pull
                    for (int i = Math.Min(parts[0], parts[2]); i <= Math.Max(parts[0], parts[2]); i++)
                    {
                        grid[(i,parts[1])] += 1;
                    }
                }
        ...
        public string OriginalSolvePart2()
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    int smallestx = Math.Min(parts[0], parts[2]);
                    int largestx = Math.Max(parts[0], parts[2]);
                    int smallesty = Math.Min(parts[1], parts[3]);
                    int largesty = Math.Max(parts[1],parts[3]);
                    int diff = largestx - smallestx;
                    if ((smallestx==parts[0] && smallesty==parts[1]) || (smallestx==parts[2] && smallesty==parts[3]))
                    {
                        //angle down
                        for (int i = 0; i <= diff; i++)
                        {
                            grid[(smallestx+i, smallesty+i)] += 1;
                        }
                    }
                    else
                    {
        ...
        public static string tdata = @"0,9 -> 5,9
8,0 -> 0,8
9,4 -> 3,4
2,2 -> 2,1
7,0 -> 7,4
...
5,5 -> 8,2";
        ...
        public static string data = @"905,103 -> 905,82
253,13 -> 253,546
578,60 -> 900,60
102,71 -> 535,71
678,419 -> 678,363
