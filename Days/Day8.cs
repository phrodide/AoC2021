...
namespace AoC2021.Days
{
    internal class Day8
    {
        ...
        public string SolvePart2()
        {
            ...
            foreach (var line in lines)
            {
                var outputValues = line.Replace(" | "," ").Trim().Split(' ');
                string one = "";//only 2 length
                string two = "";//5 length that is the odd man out.
                string three = "";//5 length that matches 1 and 7
                string four = "";//only 4 length
                string five = "";//5 length that is one less than 6
                string six = "";//6 length that doesn't match 7, 1, or 4.
                string seven = "";//only 3 length
                string eight = "";//only 7 length
                string nine = "";//6 length that matches 7,1 and 4.
                string zero = "";//6 length that matches 7 and 1 but not 4.

                one = outputValues.Where(x => x.Length==2).Select(x => new string(x.OrderBy(y => y).ToArray())).First();
                four = outputValues.Where(x => x.Length==4).Select(x => new string(x.OrderBy(y => y).ToArray())).First();
                seven = outputValues.Where(x => x.Length==3).Select(x => new string(x.OrderBy(y => y).ToArray())).First();
                eight = outputValues.Where(x => x.Length==7).Select(x => new string(x.OrderBy(y => y).ToArray())).First();
                nine = outputValues.Where(x => x.Length==6 && isSubset(x, seven) && isSubset(x, one) && isSubset(x, four)).Select(x => new string(x.OrderBy(y => y).ToArray())).First();
                zero = outputValues.Where(x => x.Length==6 && isSubset(x, seven) && isSubset(x, one) && !isSubset(x, four)).Select(x => new string(x.OrderBy(y => y).ToArray())).First();
                six = outputValues.Where(x => x.Length==6 && !isSubset(x, seven) && !isSubset(x, one) && !isSubset(x, four)).Select(x => new string(x.OrderBy(y => y).ToArray())).First();
                five = outputValues.Where(x => x.Length==5 && isSubset(six,x)).Select(x => new string(x.OrderBy(y => y).ToArray())).First();
                three = outputValues.Where(x => x.Length==5 && isSubset(x, seven) && isSubset(x, one)).Select(x => new string(x.OrderBy(y => y).ToArray())).First();
                two = outputValues.Select(x => new string(x.OrderBy(y => y).ToArray())).Where(x => x.Length==5 && x!=three && x!=five).First();

                //notice since I delegated two as "what everyone else isn't" and the order of the letters changes within the line, I had to order the letters first, then do a WHERE.
                //This code is so ugly.
                //if this was real code, I would comment the hell out of it, or at least make it understandable to the next person the looks at it.

                var finalOutput = line.Split('|')[1].Trim().Split(' ');
                int value = 0;
                foreach (var item in finalOutput)
                {
                    var item2 = new string(item.OrderBy(y => y).ToArray());
                    value *= 10;
                    if (item2==one) value+=1;
                    else if (item2==two) value+=2;
                    else if (item2==three) value+=3;
                    else if (item2==four) value+=4;
                    else if (item2==five) value+=5;
                    else if (item2==six) value+=6;
                    else if (item2==seven) value+=7;
                    else if (item2==eight) value+=8;
                    else if (item2==nine) value+=9;
                    else if (item2==zero) value+=0;
                    else
                    {
        ...
        public static string tdata = @"be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce";
        ...
        public static string data = @"fbead dcabe bcega gfbecd ecd dgac cd bedcag agebcfd fcagbe | ced cgbefad gbcaef cd
